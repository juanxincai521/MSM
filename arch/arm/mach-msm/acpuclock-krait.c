				 ...
				 const struct acpuclk_krait_params *params)
{
	...
	{
		drv.acpu_freq_tbl = kmemdup(params->pvs_tables_lge_factory[tbl_idx].table,
					    params->pvs_tables_lge_factory[tbl_idx].size,
					    GFP_KERNEL);
		BUG_ON(!drv.acpu_freq_tbl);
		drv.boost_uv = params->pvs_tables_lge_factory[tbl_idx].boost_uv;
		}
	else
	{
