...
/*                                  
	...
	/* Workarounds for die 1.1 and 1.0 and 3.0 */
	...
	} else if (pm8xxx_get_revision(chip->dev->parent) == PM8XXX_REVISION_8921_3p0) {
...
#ifdef CONFIG_MACH_LGE
	if (chip && !(chip->cool_temp_dc == 0 && chip->warm_temp_dc == 0) /* WBT : check chip is NULL */
#else /* QCT_ORG */
